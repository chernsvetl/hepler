                ...
                for (XWPFTable table : document.getTables()) {
                    for (int rowIndex = 1; rowIndex < table.getRows().size(); rowIndex++) {
                        ...
                        if (row.getTableCells().size() > dateColumnIndex) {
                            ...
                            var result  = extractFromSparqlDates(QUERY_REPORT_TRPS_4);
...
//                }
        ...
        try (FileWriter writer = new FileWriter(TRPS_4_INDIVIDUAL_TASK_FILE + ".json")) {
    ...
    public ValidatorDataIndividualTask extractFromSparql(String sparqlQuery) {
        try {
            ...
            while(results.hasNext()){
                ...
                individualStepDate = solution.getLiteral("Срок_завершения_индивидуального_этапа_из_трпс_4").toString();
                individualStepForm = solution.getLiteral("Форма_отчетности_индивидуального_этапа_из_трпс_4").toString();
            }
            qexec.close();
    ...
    public ValidatorDataDatesIndividualTask extractFromSparqlDates(String sparqlQuery) {
        try {
            ...
            while(results.hasNext()){
                QuerySolution solution = results.next();
                dateOrgStart1 = solution.getLiteral("Дата_начала_организационного_этапа_из_трпс_4").toString();
                dateDefendStart2 = solution.getLiteral("Дата_окончания_этапа_защиты_работы_из_трпс_4").toString();
            }
            qexec.close();
