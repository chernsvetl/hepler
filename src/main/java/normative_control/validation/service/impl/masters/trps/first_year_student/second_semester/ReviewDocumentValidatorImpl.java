...
import static normative_control.utils.Files.TRPS_2_REVIEW_FILE;
...
public class ReviewDocumentValidatorImpl implements ReviewDocumentValidator {
    ...
    public void validateDocxFiles(String directoryPath, ValidatorDataReview data) {
        ...
        for (File file : dir.listFiles((d, name) -> name.toLowerCase().endsWith(".docx"))) {
            ...
            try (FileInputStream fis = new FileInputStream(file);
                ...
                if (valid) {
                    ...
                    writeValidationLogs(file.getName() + DOCUMENT_SUCCESS, TRPS_2_REVIEW_FILE);
                    writeValidationLogs(NEXT_LINE, TRPS_2_REVIEW_FILE);
                } else {
                    System.out.println(ANSI_BLACK + file.getName() + DOCUMENT_ERROR + ANSI_BLACK + ANSI_RED + errorMessage + NEXT_LINE);
                    writeValidationLogs(file.getName() + DOCUMENT_ERROR + loggerInfo + NEXT_LINE, TRPS_2_REVIEW_FILE);
                }
            } catch (IOException e) {
            ...
            writeValidationLogs(SPACE, TRPS_2_REVIEW_FILE);
        }
        ...
        try (FileWriter writer = new FileWriter(TRPS_2_REVIEW_FILE + ".json")) {
        ...
        writeValidationLogs(VALIDATION_END, TRPS_2_REVIEW_FILE);
        writeValidationLogs(NEXT_LINE, TRPS_2_REVIEW_FILE);
    }
    ...
    public ValidatorDataReview extractFromSparql(String sparqlQuery) {
        try {
            ...
            while(results.hasNext()){
                ...
                dateFinishPractice = solution.getLiteral("Дата_окончания_этапа_защиты_работы_из_трпс_2").getString();
            }
            qexec.close();
