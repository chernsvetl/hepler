package normative_control.validation.service.impl.masters.iot.first_year_student.second_semester;
...
import static normative_control.utils.Files.IOT_2_INDIVIDUAL_TASK_FILE;
                ...
                for (XWPFTable table : document.getTables()) {
                    for (int rowIndex = 1; rowIndex < table.getRows().size(); rowIndex++) {
                        ...
                        if (row.getTableCells().size() > dateColumnIndex) {
                            ...
                            var result  = extractFromSparqlDates(QUERY_REPORT_IOT_2);
...
//                }
                ...
                if (valid) {
                    ...
                    writeValidationLogs(file.getName() + DOCUMENT_SUCCESS, IOT_2_INDIVIDUAL_TASK_FILE);
                } else {
                ...
                writeValidationLogs(READING_FILE_ERROR + file.getName() + ": "  + "\n" + e.getMessage(), IOT_2_INDIVIDUAL_TASK_FILE);
            }
            writeValidationLogs(SPACE, IOT_2_INDIVIDUAL_TASK_FILE);
        ...
        try (FileWriter writer = new FileWriter(IOT_2_INDIVIDUAL_TASK_FILE + ".json")) {
            gson.toJson(validationSession, writer);
        } catch (IOException e) {
            System.err.println(JSON_WRITE_ERROR + e.getMessage());
        }
        writeValidationLogs(VALIDATION_END, IOT_2_INDIVIDUAL_TASK_FILE);
        System.out.println(ANSI_BLACK + VALIDATION_END + ANSI_BLACK);
        writeValidationLogs(NEXT_LINE, IOT_2_INDIVIDUAL_TASK_FILE);
    }
    ...
    public ValidatorDataIndividualTask extractFromSparql(String sparqlQuery) {
        try {
            ...
            while(results.hasNext()){
                ...
                orgStepEndDate = solution.getLiteral("Срок_завершения_организационного_этапа_из_iot_2").toString();
                prepareAndDefendStepEndDate = solution.getLiteral("Срок_завершения_этапа_подготовки_и_защиты_отчетных_материалов_из_iot_2").toString();
                prepareAndDefendStepContent = solution.getLiteral("Содержание_работы_подготовки_и_защиты_отчетных_материалов_из_iot_2").toString();
                orgStepContent = solution.getLiteral("Содержание_работы_организационного_этапа_из_iot_2").toString();
                individualStepContent = solution.getLiteral("Содержание_работы_выполнения_этапов_индивидуального_задания_из_iot_2").toString();
                individualStepDate = solution.getLiteral("Срок_завершения_индивидуального_этапа_из_iot_2").toString();
                individualStepForm = solution.getLiteral("Форма_отчетности_индивидуального_этапа_из_iot_2").toString();
            }
            qexec.close();
    ...
    public ValidatorDataDatesIndividualTask extractFromSparqlDates(String sparqlQuery) {
        try {
            ...
            while(results.hasNext()){
                QuerySolution solution = results.next();
                dateOrgStart1 = solution.getLiteral("Дата_начала_организационного_этапа_из_iot_2").toString();
                dateDefendStart2 = solution.getLiteral("Дата_окончания_этапа_защиты_работы_из_iot_2").toString();
            }
            qexec.close();
