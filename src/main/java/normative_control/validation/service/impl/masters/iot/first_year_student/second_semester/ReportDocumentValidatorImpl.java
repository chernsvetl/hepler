...
import static normative_control.utils.Files.IOT_2_REPORT_FILE;
...
public class ReportDocumentValidatorImpl implements ReportDocumentValidator {
    ...
    public void validateDocxFiles(String directoryPath, ValidatorDataReport data) {
        ...
        for (File file : dir.listFiles((d, name) -> name.toLowerCase().endsWith(".docx"))) {
            ...
            try (FileInputStream fis = new FileInputStream(file);
                ...
                if (valid) {
                    ...
                    writeValidationLogs(file.getName() + DOCUMENT_SUCCESS, IOT_2_REPORT_FILE);
                    writeValidationLogs(NEXT_LINE, IOT_2_REPORT_FILE);
                } else {
                    System.out.println(ANSI_BLACK + file.getName() + DOCUMENT_ERROR + ANSI_BLACK + ANSI_RED + errorMessage + NEXT_LINE);
                    writeValidationLogs(file.getName() + DOCUMENT_ERROR + loggerInfo + NEXT_LINE, IOT_2_REPORT_FILE);
                }
            } catch (IOException e) {
            ...
            writeValidationLogs(SPACE, IOT_2_REPORT_FILE);
        }
        ...
        try (FileWriter writer = new FileWriter(IOT_2_REPORT_FILE + ".json")) {
        ...
        writeValidationLogs(VALIDATION_END, IOT_2_REPORT_FILE);
        writeValidationLogs(NEXT_LINE, IOT_2_REPORT_FILE);
    }
