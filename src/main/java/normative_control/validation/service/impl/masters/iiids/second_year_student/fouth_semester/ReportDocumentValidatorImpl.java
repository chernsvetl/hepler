...
public class ReportDocumentValidatorImpl implements ReportDocumentValidator {
    ...
    public void validateDocxFiles(String directoryPath, ValidatorDataReport data) {
        ...
        for (File file : dir.listFiles((d, name) -> name.toLowerCase().endsWith(".docx"))) {
            ...
            try (FileInputStream fis = new FileInputStream(file);
                ...
                if (valid) {
                    ...
                    writeValidationLogs(file.getName() + DOCUMENT_SUCCESS, IIIDS_4_REPORT_FILE);
                    writeValidationLogs(NEXT_LINE, IIIDS_4_REPORT_FILE);
                } else {
                    System.out.println(ANSI_BLACK + file.getName() + DOCUMENT_ERROR + ANSI_BLACK + ANSI_RED + errorMessage + NEXT_LINE);
                    writeValidationLogs(file.getName() + DOCUMENT_ERROR + loggerInfo + NEXT_LINE, IIIDS_4_REPORT_FILE);
                }
            } catch (IOException e) {
            ...
            writeValidationLogs(SPACE, IIIDS_4_REPORT_FILE);
        }
        ...
        try (FileWriter writer = new FileWriter(IIIDS_4_REPORT_FILE + ".json")) {
        ...
        writeValidationLogs(VALIDATION_END, IIIDS_4_REPORT_FILE);
        writeValidationLogs(NEXT_LINE, IIIDS_4_REPORT_FILE);
    }
