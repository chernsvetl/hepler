...
import static normative_control.utils.Files.TRPS_2_REPORT_FILE;
import static normative_control.utils.ValidationPaths.MODEL_FILENAME;
import static normative_control.validation.service.utils.Constants.CITY_YEAR_PATTERN;
import static normative_control.validation.service.utils.Constants.CM_TO_TWIPS;
import static normative_control.validation.service.utils.Constants.EXPECTED_INDENTATION_CM;
import static normative_control.validation.service.utils.Constants.EXPECTED_LINE_SPACING;
import static normative_control.validation.service.utils.Constants.LINE_TO_TWIPS;
import static normative_control.validation.service.utils.Constants.TITLE_START;
...
public class ReportDocumentValidatorImpl implements ReportDocumentValidator {
    ...
    public void validateDocxFiles(String directoryPath, ValidatorDataReport data) {
        ...
        for (File file : dir.listFiles((d, name) -> name.toLowerCase().endsWith(".docx"))) {
            ...
            try (FileInputStream fis = new FileInputStream(file);
                ...
                if (valid) {
                    ...
                    writeValidationLogs(file.getName() + DOCUMENT_SUCCESS, TRPS_2_REPORT_FILE);
                    writeValidationLogs(NEXT_LINE, TRPS_2_REPORT_FILE);
                } else {
                    System.out.println(ANSI_BLACK + file.getName() + DOCUMENT_ERROR + ANSI_BLACK + ANSI_RED + errorMessage + NEXT_LINE);
                    writeValidationLogs(file.getName() + DOCUMENT_ERROR + loggerInfo + NEXT_LINE, TRPS_2_REPORT_FILE);
                }
            } catch (IOException e) {
            ...
            writeValidationLogs(SPACE, TRPS_2_REPORT_FILE);
        }
        ...
        try (FileWriter writer = new FileWriter(TRPS_2_REPORT_FILE + ".json")) {
        ...
        writeValidationLogs(VALIDATION_END, TRPS_2_REPORT_FILE);
        writeValidationLogs(NEXT_LINE, TRPS_2_REPORT_FILE);
    }
    @Override
