package normative_control.validation.service.impl.bachelors.knis.fourth_year_students.seventh_semester;
...
import static normative_control.utils.Files.KNIS_7_INDIVIDUAL_TASK_FILE;
...
public class IndividualTaskDocumentValidatorImpl implements IndividualTaskDocumentValidator {
    ...
    public void validateDocxFiles(String directoryPath, ValidatorDataIndividualTask data) {
        ...
        for (File file : dir.listFiles((d, name) -> name.toLowerCase().endsWith(".docx"))) {
            ...
            try (FileInputStream fis = new FileInputStream(file);
                ...
                for (XWPFTable table : document.getTables()) {
                    for (int rowIndex = 1; rowIndex < table.getRows().size(); rowIndex++) {
                        ...
                        if (row.getTableCells().size() > dateColumnIndex) {
                            ...
                            var result  = extractFromSparqlDates(QUERY_REPORT_KNIS_7);
                ...
                if (valid) {
                    ...
                    writeValidationLogs(file.getName() + DOCUMENT_SUCCESS, KNIS_7_INDIVIDUAL_TASK_FILE);
                } else {
                ...
                writeValidationLogs(READING_FILE_ERROR + file.getName() + ": "  + "\n" + e.getMessage(), KNIS_7_INDIVIDUAL_TASK_FILE);
            }
            writeValidationLogs(SPACE, KNIS_7_INDIVIDUAL_TASK_FILE);
        ...
        try (FileWriter writer = new FileWriter(KNIS_7_INDIVIDUAL_TASK_FILE + ".json")) {
            gson.toJson(validationSession, writer);
        } catch (IOException e) {
            System.err.println(JSON_WRITE_ERROR + e.getMessage());
        }
        writeValidationLogs(VALIDATION_END, KNIS_7_INDIVIDUAL_TASK_FILE);
        System.out.println(ANSI_BLACK + VALIDATION_END + ANSI_BLACK);
        writeValidationLogs(NEXT_LINE, KNIS_7_INDIVIDUAL_TASK_FILE);
    }
    ...
    public ValidatorDataDatesIndividualTask extractFromSparqlDates(String sparqlQuery) {
        try {
            ...
            while(results.hasNext()){
                QuerySolution solution = results.next();
                dateOrgStart1 = solution.getLiteral("Дата_начала_организационного_этапа_из_книс_7").toString();
                dateDefendStart2 = solution.getLiteral("Дата_окончания_этапа_защиты_работы_из_книс_7").toString();
            }
            qexec.close();
